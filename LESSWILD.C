...
dirlook()
{
	sprintf(syscall,"DIR %s /A:-D /B > zzzzzzzz",filename);
	system(syscall);
	...
	while(feof(infil_ptr) == 0)
	{
		...
		p2++;
		if((strncmp(p2,"EXE",3) != 0) && (strncmp(p2,"COM",3) != 0) && (strncmp(p2,"OBJ",3) != 0))  
		{
			len = strcspn(buf2," ");
			strncpy(arr[sub],buf2,len);
			len--;
			arr[sub][len]='\0';
			sub++;
		}
...
whattodo()
{
	switch (tolower(keyb))
	{
		...
		case '+'  : hohoflag++;fclose(infil_ptr);linecnt=0;cnt=0;sub++;openfiles();break;
...
whattodoatend()
{
	switch (tolower(keyb))
	{
		...
		case '+'  : hohoflag++;fclose(infil_ptr);linecnt=0;cnt=0;sub++;openfiles();loop();break;
		case 'b'  : hohoflag=0;back(scrn);break;
		case 'e'  : hohoflag=0;end();break;
