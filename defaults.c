...
int cnt1, cnt2, i, commacnt, len, dotlen, ccctr, flag;
int *t;
...
samerec()
{
	...
	else
	{
		...
		minsdouble *= 60.0;
		if(modf(minsdouble,t) > 0.5)
		{		
			minsint = (int) ceil(minsdouble);
		}
		else
		{
			minsint = (int) floor(minsdouble);
		}
...
/*		fprintf(out,"%06s,%s,%s,%s,%s,%s\n",staffno,name,costcentre,outputcode,hourschar,minschar); */
	...
	else
	{
		...
		strncpy(hourschar,p2,dotlen);
		p2 += dotlen;
		strcpy(minshold,p2);
		minsdouble = atof(minshold); 
		minsdouble *= 60.0;
		if(modf(minsdouble,t) > 0.50)
		{		
			minsint = (int) ceil(minsdouble);
		}
		else
