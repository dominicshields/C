...
{
	static int all_targets_hit;
	static int byte;
	register int byte_num;
	static long compressed_size;
	static int extra_field_length;
	static char file_name [1024];
	static int file_name_length;
	static int file_num;
	static int flags;
	static unsigned char header [NFILES] [12];
	static unsigned long key [3];
	static int num_enciphered;
	static char password [255];
	static char *password_ptr;
	FILE *passwords;
	static int possibility_found;
	static unsigned long signature;
	static unsigned char target [NFILES];
	register unsigned short tem;
	FILE *zip_file;

	if (argc == 2) {
		printf ("\n");
		if ((zip_file = fopen (argv [1], "rb")) == NULL)
			printf ("Fatal error:  cannot open %s for input.\n", argv [1]);
		else {
			num_enciphered = 0;
			while ((num_enciphered < NFILES) &&
			       fread ((void *) &signature, 4, 1, zip_file) &&
				...
				if (((int) flags) & 1) {
					...
					fread ((void *) &file_name [0], 1, file_name_length, zip_file);
			...
			if (num_enciphered == 0)
			...
			else if (num_enciphered < NFILES)
				printf ("Fewer than %d file%s enciphered in %s.\n", NFILES, (NFILES > 1) ? "s are" : "is" , argv [1]);
			else if ((passwords = fopen ("PKCRACK.DAT", "r")) == NULL)
				printf ("Fatal error:  cannot open PKCRACK.DAT for input.\n");
			else {
				possibility_found = FALSE;
				byte_num = 0;
				while (fgets (&password [0], 255, passwords) != NULL) {
					password [strlen (&password [0]) - 1] = '\0';
					all_targets_hit = TRUE;
					file_num = 0;
					while (all_targets_hit &&
					       (file_num < num_enciphered)) {
						key [0] = 305419896L;
						key [1] = 591751049L;
						key [2] = 878082192L;
						...
						while (*password_ptr != '\0') {
							...
							key [2] = crc32 (key [2], (int) (key [1] >> 24));
						}
						for (byte_num = 0; byte_num < 12; byte_num++) {
							tem = (unsigned short) key [2] | 2;
				...
				if (! possibility_found)
					printf ("\nSorry, the key was not found on PKCRACK.DAT.\n");
				fclose (passwords);
			}
		}
	} else {
		printf ("\n     PKCRACK searchs PKCRACK.DAT for the key used to encipher files in a\n");
		printf ("PKZIP archive.\n");
		printf ("\n     Usage:  PKCRACK <archive-descriptor>\n");
		printf ("\n     Example:  PKCRACK SAMPLE.ZIP\n");
		printf ("\n     At least %d file%s in the archive must have been enciphered.  The\n", NFILES, (NFILES > 1) ? "s" : "");
		printf ("first %d enciphered files must have been enciphered with the same key.\n", NFILES);
	}
	return;
}
